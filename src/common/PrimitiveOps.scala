...
trait LiftPrimitives {
  ...
  implicit def chainIntToRepLong[A:Manifest](x: A)(implicit c: A => Rep[Int]): Rep[Long] = repIntToRepLong(c(x))
  implicit def chainIntToRepDouble[A:Manifest](x: A)(implicit c: A => Rep[Int]): Rep[Double] = repIntToRepDouble(c(x))
  implicit def chainFloatToRepDouble[A:Manifest](x: A)(implicit c: A => Rep[Float]): Rep[Double] = repFloatToRepDouble(c(x))
  implicit def chainLongToRepDouble[A:Manifest](x: A)(implicit c: A => Rep[Long]): Rep[Double] = repLongToRepDouble(c(x))
}
...
trait PrimitiveOpsExp extends PrimitiveOps with EffectExp {
  ...
  def long_plus(lhs: Exp[Long], rhs: Exp[Long])(implicit pos: SourceContext) : Exp[Long] = (lhs, rhs) match {
    case (Const(0l), r) => r
    case (l, Const(0l)) => l
    case (Const(x), Const(y)) => Const(x+y)
    case _ => LongPlus(lhs,rhs)
  }
  def long_minus(lhs: Exp[Long], rhs: Exp[Long])(implicit pos: SourceContext) : Exp[Long] = (lhs, rhs) match {
    case (l, Const(0l)) => l
    case (Const(x), Const(y)) => Const(x-y)
    case _ => LongMinus(lhs, rhs)
  }
  def long_times(lhs: Exp[Long], rhs: Exp[Long])(implicit pos: SourceContext) : Exp[Long] = (lhs, rhs) match {
    case (l@Const(0l), r) => l
    case (l, r@Const(0l)) => r
    case (Const(1l), r) => r
    case (l, Const(1l)) => l
    case (Const(x), Const(y)) => Const(x*y)
    case _ => LongTimes(lhs, rhs)
  }
