...
trait LoopsExp extends Loops with BaseExp with EffectExp {
  ...
  case class SimpleLoop[A](val size: Exp[Long], val v: Sym[Long], val body: Def[A]) extends AbstractLoop[A]
  
  def simpleLoop[A:Manifest](size: Exp[Long], v: Sym[Long], body: Def[A])(implicit pos: SourceContext): Exp[A] = SimpleLoop(size, v, body)
  ...
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case SimpleLoop(s,v,body: Def[A]) => simpleLoop(f(s),f(v).asInstanceOf[Sym[Long]],mirrorFatDef(body,f))
    case Reflect(SimpleLoop(s,v,body: Def[A]), u, es) if u == Control() => reflectMirrored(Reflect(SimpleLoop(f(s),f(v).asInstanceOf[Sym[Long]],mirrorFatDef(body,f)), mapOver(f,u), f(es)))(mtype(manifest[A]), pos) 
...
trait LoopsFatExp extends LoopsExp with BaseFatExp {
  ...
  case class SimpleFatLoop(val size: Exp[Long], val v: Sym[Long], val body: List[Def[Any]]) extends AbstractFatLoop
